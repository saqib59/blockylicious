...
<div <?= $block_wrapper_attributes; ?>>
    <div class="curve top-curve" style="display: <?= $attributes['enableTopCurve']? "block" : "none";?>; transform: <?= $topTransform; ?>; height: <?= $attributes['topHeight']; ?>px;">
        <svg style="height: <?= $attributes['topHeight']; ?>px;
            width: <?= $attributes['topWidth']; ?>%;" 
            preserveAspectRatio="none" viewBox="0 0 1200 120">
            <path fill="<?= $attributes['topColor'] ?? "white"; ?>"
                d=<?= $attributes['topFlipY'] ? $invertedPath : $normalPath; ?>></path>
        </svg>
    </div>
        <?php echo $content; ?>
    <div class="curve bottom-curve" style="display: <?= $attributes['enableBottomCurve']? "block" : "none";?>; transform: <?= $bottomTransform; ?>; height: <?= $attributes['bottomHeight']; ?>px;">
        <svg style="height: <?= $attributes['bottomHeight']; ?>px;
            width: <?= $attributes['bottomWidth']; ?>%;" 
            preserveAspectRatio="none" viewBox="0 0 1200 120">
            <path fill="<?= $attributes['bottomColor'] ?? "white"; ?>"
                d=<?= $attributes['bottomFlipY'] ? $invertedPath : $normalPath; ?>></path>
        </svg>
    </div>
</div>